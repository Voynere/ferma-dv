...
function fdv_yandex_get_price_for_eda( WC_Product $product ) : float {
    ...
    // 3) Woo
    return max(0.0, (float)$product->get_price());
}
/**
...
function fdv_yandex_nomenclature_prices(WP_REST_Request $request) {
    ...
    // Получаем товары
    $res = fdv_yandex_get_items_for_composition($place_id, $limit, $offset);
    ...
    foreach ($res['items'] as $item) {
        ...
        // Получаем цену для Яндекс Еды
        $price = fdv_yandex_get_price_for_eda($product);

        // Преобразуем в копейки (если Яндекс ждёт целое число)
        $price_in_cents = (int) round($price);

        $prices[] = [
            'id'    => (string) $product_id,
            'price' => (int) $price_in_cents, // или float, если у них не “копейки”
        ];
...
function fdv_yandex_oauth_token_endpoint( WP_REST_Request $request ) {
    ...
    if ( $client_id !== FDV_YA_CLIENT_ID || $client_secret !== FDV_YA_CLIENT_SECRET ) {
        yandex_log([
            ...
            'got_secret_len'  => is_string($client_secret) ? strlen($client_secret) : null,
        ], 'TOKEN_401');
...
function fdv_yandex_nomenclature_availability(WP_REST_Request $request) {
    ...
    // Получаем товары
    $res = fdv_yandex_get_items_for_composition($place_id, 5000, 0);
    $items = $res['items'];
 ...
 * ==========================
 */
define('MS_LOGIN', 'fenikc@fermadv');
define('MS_PASSWORD', 'Wsd19862011!!');
if (!defined('MS_BASE')) {
    define('MS_BASE', 'https://api.moysklad.ru/api/remap/1.2');
}

if (!defined('MS_TOKEN')) {
    define('MS_TOKEN', 'ec8b476e6c5c2c31c03257898db4393c1bd0e42a'); // токен без двоеточия
}
